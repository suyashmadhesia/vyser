    ...
    def lex(self):
        ...
        if self.current.isdigit():
            ...
            try:
                ...
                self._diagnostics.report_invalid_number(TextSpan(start, length), self._text, int)
        ...
        if self.current == '$':
            self._next()
            start: int = self._position
            while self.current.isalpha() or self.current == '.' or self.current == '_' or self.current.isdigit(): # it is also considering the digits for array indices
                self._next()
            text: str = self._text[start: self._position]
        ...
        if self.current == "<":
            if self._lookahead == "=":
                self._next()
                self._next()
                return SyntaxToken(SyntaxKind.LesserThanEqualToken, self._position, "<=")
            self._next()
        ...
        if self.current == ">":
            if self._lookahead == "=":
                self._next()
                self._next()
                return SyntaxToken(SyntaxKind.GreaterThanEqualToken, self._position, ">=")
            self._next()
        ...
        self._next()
        return SyntaxToken(SyntaxKind.BadToken, self._position, self._text[self._position-1: self._position + 1])